...
Frames loop_frames (const char* input_filepath, unsigned int start_time, unsigned int end_time,
          unsigned int min_length, const char* output_filepath) {
    /* Used to store some file metadata, required by sf_open() */
    Frames frames;
    SF_INFO info = {0,0,0,0,0,0};
    ...
    SNDFILE* fout = sf_open(output_filepath, SFM_WRITE, &output_info);
    sf_writef_short(fout, extended, total_len);
    frames.frames=extended;
    frames.frame_len=total_len;
...
int main (int argc, char** argv) {
    ...
    int res = loop("recycling.wav", 0, 77, 600, OUTPUT);
    /*
    ...
    */
    ...
    Frames frames=loop_frames("recycling.wav", 0, 77, 600, OUTPUT);
